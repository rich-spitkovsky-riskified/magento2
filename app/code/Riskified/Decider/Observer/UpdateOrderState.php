...
use Riskified\Decider\Config\Source\DeclinedState;
...
class UpdateOrderState implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $this->logger->log(
            "Checking if should update order '"
            . $order->getId() . "' from state: '"
            . $currentState . "' and status: '"
            . $currentStatus
        );

        $this->logger->log(
            "Data received from riskified: status: "
            . $riskifiedStatus . ", old_status: "
            . $riskifiedOldStatus . ", description: "
            . $description
        );
        ...
        $this->logger->log(
            ...
            . " and Transport Error Status Code : "
        ...
        switch ($riskifiedStatus) {
            ...
            case 'submitted':
                if (
                    $currentState == \Magento\Sales\Model\Order::STATE_PROCESSING
                    || (
                        $currentState == \Magento\Sales\Model\Order::STATE_HOLDED
                        && $currentStatus == $this->apiOrderConfig->getTransportErrorStatusCode()
                    )
        ...
        if (
            ...
            if ($newState == \Magento\Sales\Model\Order::STATE_CANCELED) {
                $this->logger->log(
                    "Order '" . $order->getId()
                    . "' should be canceled - calling cancel method"
                );
                $order->cancel();
            }

            if (
                $riskifiedStatus != 'approved'
                || !$this->apiConfig->isAutoInvoiceEnabled()
            ) {
                ...
                $order->addStatusHistoryComment(
                    $this->formatDescription($riskifiedStatus, $description)
                );
                $this->logger->log(
                    "Updated order '"
                    . $order->getId() . "' to: state:  '"
                    . $newState."', status: '".$newStatus."', description: '"
                    . $description
                );
                $changed = true;
            }
            ...
            if (
                ...
                $this->logger->log(
                    ...
                    " history comment to: " . $description
                );
                $order->addStatusHistoryComment(
                    $this->formatDescription($riskifiedStatus, $description)
                );
            ...
            $this->logger->log(
                "No update to state, status, comments is required for "
                . $order->getId()
            );
        }
    ...
    protected function formatDescription($apiStatus, $apiComment)
    {
        if ($apiStatus == DeclinedState::API_STATUS) {
            return __("Declined by Riskified");
        }
        return $apiComment;
    }
}
